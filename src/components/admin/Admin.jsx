...
function Admin() {
  ...
  const clickFunction=(e)=>{
    e.preventDefault();
    if (inputValue.email===log && inputValue.password === passw ){
      navigate("/panel");
    }
    else{
      setError(false);
  ...
  return (
      ...
      <form onClick={clickFunction} className='admin__panel'>
        ...
        <button className='admin__panel--r__button' type='submit'> {t("Enter")} </button>

        <Link className='admin__panel--r__link'>  {t("Forgot your login or passwor ?")} </Link>
        </div>
      </form>
    </div>
